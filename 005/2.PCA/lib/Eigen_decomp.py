...
class Eigen_decomp:
    ...
    def __init__(self,x,f,mean,v):
        ...
        #self.C=np.dot(self.U,np.nan_to_num(f)-mean)  # compute the eigen-vectors coefficients.
        self.t[np.argwhere(np.isnan(f))]= mean[np.argwhere(np.isnan(f))]
        self.C=np.dot(self.U,np.nan_to_num(self.t)-mean)  # compute the eigen-vectors coefficients.
