...
class Eigen_decomp:
    ...
    def __init__(self,x,f,mean,v):
        ...
        #create a temp vector. copy f and replace all NaN values with corresponding values from mean 
        #so that these values do not add any weights to coefficient computation 
        self.t=f
        self.t[np.argwhere(np.isnan(f))] = mean[np.argwhere(np.isnan(f))]
        self.C=np.dot(self.U,np.nan_to_num(self.t)-mean)  # computer the eigen-vectors coefficients.
